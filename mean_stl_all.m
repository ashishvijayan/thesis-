% by ashish
clear;clc
filePath = 'C:\Users\IITGN\Desktop\ashish\expt\1revised_participants\all_stl.csv'; 
%filePath = '/home/ashish/Desktop/thesis/expt/all_stl.csv';
...
for c = 1:length(all_stl(1,:))
    ...
    if mod(c,2) ~= 0 %if the column index is odd
        % odd columns has the stl values
        % disp(c)
        NJ_pos = 0;
        NJ_neg = 0;
        JT_pos = 0;
        ...
        % Create a logical array indicating rows to nan
        rows_to_NaN = all_stl(:,c) > 90000 | all_stl(:,c) < -90000 ;

        all_stl(rows_to_NaN, :) = nan;
        for r = 1: length(all_stl(:,c))
            %iterate through each row of the odd column
...
% f = gcf;
% filename = fullfile('C:\Users\IITGN\Desktop\ashish\expt\1revised_participants', 'all_mean_stlv3.png');
